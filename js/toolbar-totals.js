...
export function renderBudgetTotals(targetEl, totals, currency = 'ZAR') {
  ...
  targetEl.innerHTML = `
      <span class="toolbar-total-item"><span class="label">Actual Net:</span> <span class="value ${totals.actualNet >= 0 ? 'positive' : 'negative'}">${actualNet}</span></span>
      <span class="toolbar-total-item"><span class="label">Money In:</span> <span class="value positive">${moneyIn}</span></span>
      <span class="toolbar-total-item"><span class="label">Money Out:</span> <span class="value negative">${moneyOut}</span></span>
      <span class="toolbar-total-item"><span class="label">Net:</span> <span class="value ${totals.net >= 0 ? 'positive' : 'negative'}">${net}</span></span>
      <span class="toolbar-total-item"><span class="label">Planned Outstanding:</span> <span class="value negative">${plannedOutstanding}</span></span>
      <span class="toolbar-total-item"><span class="label">Planned Net Balance:</span> <span class="value ${totals.plannedNetBalance >= 0 ? 'positive' : 'negative'}">${plannedNetBalance}</span></span>
      <span class="toolbar-total-item"><span class="label">Unplanned:</span> <span class="value negative">${unplanned}</span></span>
    `;
}
