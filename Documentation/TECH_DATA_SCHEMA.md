...
scheduled date is generated by that rule, its manual identity is unique, and the
linked key is collision-free. Resolved notes are removed; unresolved notes and
recovery records remain in `migrationReport`. The downloadable report embeds the
original report as `resolvedMigrationReport` and lists every applied relink.
